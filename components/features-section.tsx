...
const AnimatedChatDemo = ({ isActive }: { isActive: boolean }) => {
  const [messages, setMessages] = useState([
    { text: "Sleep Mode ready. What should I protect?", isBot: true, visible: false },
    { text: "Guard BTC, ETH and SOL overnight", isBot: false, visible: false },
    { text: "Balanced protection armed. I will hedge if danger rises.", isBot: true, visible: false },
  ])
  ...
  useEffect(() => {
    ...
    const scenarios = [
      [
        { text: "Sleep Mode ready. What should I protect?", isBot: true },
        { text: "Guard BTC, ETH and SOL overnight", isBot: false },
        { text: "Balanced protection armed. I will hedge if danger rises.", isBot: true },
      ],
      [
        { text: "ETF flows turned negative. Risk score is 68.", isBot: true },
        { text: "What action is ready?", isBot: false },
        { text: "A SoDEX stop ladder is prepared for wallet approval.", isBot: true },
      ],
      [
        { text: "Good evening. Whale sell pressure detected.", isBot: true },
        { text: "Protect profits but do not panic sell", isBot: false },
        { text: "Profit lock set to 18%. Hedge waits for confirmation.", isBot: true },
      ],
    ]
...
const AnimatedPhoneDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32 flex items-center justify-center relative">
      <div className="absolute top-2 right-2 text-xs text-slate-500 font-medium">Checks: {callCount + 1}</div>
      <div className="relative">
        ...
        {callState === "answered" && (
          <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
            <div className="bg-blue-100 px-2 py-1 rounded text-xs text-blue-700 whitespace-nowrap">Risk checked</div>
          </div>
        )}
      </div>
    </div>
...
const AnimatedCalendarDemo = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4 h-32">
      ...
      {booked && (
        <div className="mt-2 text-xs text-green-600 font-medium animate-fade-in">Protection window armed for 02:00</div>
      )}
    </div>
  )
}
...
const AnimatedEmailDemo = ({ isActive }: { isActive: boolean }) => {
  const [emails, setEmails] = useState([
    { subject: "ETF outflow alert", status: "unread" },
    { subject: "SOL momentum fade", status: "unread" },
    { subject: "BTC hedge prepared", status: "unread" },
  ])
...
const AnimatedLeadsDemo = ({ isActive }: { isActive: boolean }) => {
  const [leads, setLeads] = useState([
    { name: "BTC", score: 0, qualified: false },
    { name: "ETH", score: 0, qualified: false },
    { name: "SOL", score: 0, qualified: false },
  ])
...
const AnimatedIntegrationsDemo = ({ isActive }: { isActive: boolean }) => {
  const [connections, setConnections] = useState([
    { name: "SoSoValue", connected: false },
    { name: "SoDEX", connected: false },
    { name: "ValueChain", connected: false },
    { name: "Wallet", connected: false },
  ])
...
const features = [
  {
    title: "AI Sleep Mode",
    description:
      "A simple protection command turns on overnight monitoring, mode-aware hedging, and a clear morning safety trail.",
    demo: AnimatedChatDemo,
    size: "large",
  },
  {
    title: "Market Danger Score",
    description:
      "NightWatch blends price movement, ETF pressure, news shock terms, and sector rotation into one actionable score.",
  ...
  },
  {
    ...
    description:
      "Define when the guardian is allowed to act and which portfolio policies apply while the trader is offline.",
  ...
  },
  {
    title: "Smart Alerts",
    description:
      "Get concise explanations for risk alerts, stop triggers, profit locks, and SoDEX execution attempts.",
  ...
  },
  {
    ...
    description:
      "Each holding gets an evolving risk score so protection actions target the weakest positions first.",
    demo: AnimatedLeadsDemo,
    size: "medium",
  },
  {
    title: "On-chain Execution Stack",
    description:
      "SoSoValue powers intelligence, SoDEX powers trading, and ValueChain wallet signatures keep the user in control.",
        ...
        <div className="max-w-7xl mx-auto relative">
          ...
          >
            ...
            <p className="text-base sm:text-lg md:text-xl text-slate-600 max-w-3xl mx-auto font-light leading-relaxed">
              Watch NightWatch turn market data into a protection plan that can be signed and routed through SoDEX.
            </p>
          </div>
